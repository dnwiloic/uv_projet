...
def predict_crop():
    try:
        ...
        input_data = [float(data[key]) for key in [
            'nitrogen', 'phosphorus', 'potassium', 'temperature', 'humidity', 'ph', 'rainfall'
        ]]

        model = load_model()
        pred = predict_crop_values(model, [input_data])
...
def get_weather():
    ...
    lat = request.args.get('lat', '5.4527273')  # Default latitude if none provided
    lon = request.args.get('lon', '10.0618887')  # Default longitude if none provided
